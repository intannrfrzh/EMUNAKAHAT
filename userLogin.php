...
<body>
  ...
  <form action="action_page.php" method="post">
    ...
    <div class="container">
      ...
      <input type="text" placeholder="Masukkan No. Kad Pengenalan" name="ic" id="applicant_IC" required pattern="\d{6}-\d{2}-\d{4}" title="Format: xxxxxx-xx-xxxx">

      <label for="applicant_password"><b>Kata Laluan</b></label>
      <input type="password" placeholder="Masukkan Kata Laluan" name="password" id="applicant_password" required minlength="8">
  ...
  <script>
    document.getElementById('loginForm').addEventListener('submit', function(event) {
      ...
      // Get form values
      const ic = document.getElementById('applicant_IC').value;
      const password = document.getElementById('applicant_password').value;
      const accountType = document.getElementById('account-type').value;

      // Validate IC format
      const icPattern = /^\d{6}-\d{2}-\d{4}$/;
      if (!icPattern.test(ic)) {
        alert('Format No. Kad Pengenalan tidak sah. Sila masukkan dalam format xxxxxx-xx-xxxx.');
        return;
      }

      // Validate password length
      if (password.length < 8) {
        alert('Kata Laluan mesti mempunyai sekurang-kurangnya 8 aksara.');
        return;
      }

      // Redirect based on account type
      if (accountType === 'Staf') {
        window.location.href = '../staffIncentive/listView.php';
      } else {
        window.location.href = 'userProfileView.php';
      }
    });
